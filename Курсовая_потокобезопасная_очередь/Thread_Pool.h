...
class Thread_Pool
{
	...
	public:
		...
		//метод work Ч выбирает из очереди очередную задачу и исполн€ет еЄ. Ётот метод передаЄтс€ конструктору потоков дл€ исполнени€;
		void work();
		...
		void submit(std::vector<std::function<void()>> fun);
};