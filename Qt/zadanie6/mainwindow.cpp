...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        auto future1 = [this]{return concurRace1->DoWork(&number, ui->rb_mutexOn->isChecked(), countFinish);};
        auto future2 = [this]{return concurRace2->DoWork(&number, ui->rb_mutexOn->isChecked(), countFinish);};
        QFuture<void> future = QtConcurrent::run(future1).then(future2);
    }
    else{
