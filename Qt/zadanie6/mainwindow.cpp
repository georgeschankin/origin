...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        QFuture<void> future = {QtConcurrent::run(concurRace1->DoWork, number, ui->rb_mutexOn->isChecked(), countFinish),
                                QtConcurrent::run(concurRace2->DoWork, number, ui->rb_mutexOn->isChecked(), countFinish)};
    }
    else{
