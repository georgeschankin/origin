...
void FindMin2(Container& a, int i, const int n, std::promise<T> prom)
{
    int j, min;
    min = i;
    for (j = i + 1; j < n; j++)
...
void SelectionSort2(Container& arr, T& elem, const int size)
{
    for (int i = 0; i < size - 1; i++)
    {
        ...
        auto res = std::async(std::launch::async, FindMin2<Container, T>, std::ref(arr), i, size, std::move(p));
        //f.wait();
        int min_index = f.get();
        if (min_index != i)
        {
...
int main()
{
    ...
    SelectionSort2(myList1, myList1[1],  myList1.size());
