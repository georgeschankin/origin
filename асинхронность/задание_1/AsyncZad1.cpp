...
int main()
{
    ...
    //std::thread t1(SelectionSort, myList, 5);
    //t1.join();

    //std::thread t1(SelectionSort, myList, 5); //способ1 стандартные функции и через массив
    //SelectionSort2(myList, myList.size(), std::move(p)); //способ 2 через контейнеры
    //std::thread t2(SelectionSort2, myList, myList.size()), std::move(p));
