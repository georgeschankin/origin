...
{   
    ...
    {
        std::string substr_upequial; //подстрока до = (название)
        std::string substr_afterequial; //подстрока после = (значение)
        ...
        {           
            ...
            if (str.find(';') == std::string::npos)
            {
                ...
                {
                    substr_upequial = str.substr(0, str.find('='));
                    substr_afterequial = str.substr(str.find('=') + 1);
                }
            }
            else
            {
        ...
        auto pair_name_val = std::make_pair(nameval[str_name_val], val[str_name_val]);
        auto section_pair_name_val = std::make_pair(section[str_sec], pair_name_val);
        section_name_value.insert(section_pair_name_val);
